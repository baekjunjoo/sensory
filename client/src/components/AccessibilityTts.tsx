...
export function AccessibilityTts({ content }: { content: ReaderContent }) {
  ...
  const applyHighlightPreset = (preset: "light" | "dark" | "large") => {
    if (preset === "light") {
      setHighlightText("black");
      setHighlightSize("large");
      setHighlightBackground("contrast");
      setStatus("밝은 고대비 프리셋을 적용했어요.");
      return;
    }
    if (preset === "dark") {
      setHighlightText("navy");
      setHighlightSize("large");
      setHighlightBackground("night");
      setStatus("어두운 고대비 프리셋을 적용했어요.");
      return;
    }
    setHighlightText("black");
    setHighlightSize("xlarge");
    setHighlightBackground("contrast");
    setStatus("큰 글자 고대비 프리셋을 적용했어요.");
  };
  ...
  const loadTextFile = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setScope("file");
      setStatus(`${file.name}을(를) 기기에서 불러왔어요. 서버로 전송되지 않아요.`);
    } catch (error) {
      setStatus(error instanceof Error ? error.message : "텍스트 파일을 불러오지 못했어요.");
    } finally {
      event.target.value = "";
    }
  };
