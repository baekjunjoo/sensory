...
export function AccessibilityTts({ content }: { content: ReaderContent }) {
  ...
  return <aside className="accessibility-tts" aria-label="다국어 접근성 음성 읽기">
    ...
    {uploadedText && <div className="accessibility-tts__file-preview" role="region" aria-label="불러온 텍스트 미리보기"><b>{uploadedFileName}</b><p>{uploadedText}</p></div>}
    <div className="accessibility-tts__transcript" role="region" data-highlight-text={highlightText} data-highlight-size={highlightSize} data-highlight-background={highlightBackground} aria-label="문장별 읽기 진행"><b>읽는 문장</b><p>{transcript.map((sentence, index) => <span ref={index === sentenceIndex ? activeSentence : undefined} className={index === sentenceIndex ? "is-reading" : ""} key={`${index}-${sentence}`}>{sentence}</span>)}</p></div><p role="status" aria-live="polite" className="accessibility-tts__status">{status}</p>
  </aside>;
}
