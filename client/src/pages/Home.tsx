...
import { Archive, ArrowRight, BookOpenCheck, ChartNoAxesCombined, Check, ChevronRight, Headphones, House, MailOpen, Menu, PanelsTopLeft, Send, Stamp, Volume2, VolumeX, X } from "lucide-react";
import { DotPadConnection } from "@/components/DotPadConnection";
import { applyCharacterTheme, characters, dailyLessons, friendHeroSceneIndex, loadCharacterTheme, type CharacterKey, type DailyLesson } from "@/lib/dailyContent";
import { getCurrentStreak, loadLearningRoutine, markDotPadFrameSent, markLessonCompleted, markLessonForReview, markLessonOpened, markTactileExplored, saveLearningRoutine, type LearningRoutine } from "@/lib/dailyRoutine";
...
function CompletionStamp({ lesson, streak, archiveHref, onNext, onReview }: { lesson: DailyLesson; streak: number; archiveHref: string; onNext: () => void; onReview: () => void }) {
  return <section className="completion-stamp" aria-labelledby="completion-title"><div className="completion-copy"><span className="capsule-label"><i /><i /><i /> TODAY&apos;S SHEET <b>DONE</b></span><h3 id="completion-title">오늘의 한 장을<br /><em>완성했어요.</em></h3><p>{streak}일째 이어가는 손끝의 습관이에요. 오늘의 촉각 학습지는 보관함에 안전하게 담겼어요.</p><div className="completion-actions"><a href={archiveHref}><Archive size={16} />보관함에서 보기</a><button type="button" onClick={onNext}>내일의 한 장 미리 보기 <ArrowRight size={16} /></button><button type="button" onClick={onReview}>학습지 다시 보기</button></div></div><div className="completion-emblem" aria-label="완료 도장과 촉각 스티커"><span className="stamp-mark">완료</span><span className="sticker-chip"><Stamp size={13} />촉각 스티커 +1</span></div></section>;
}
...
export default function Home() {
  ...
  const [lessonIndex, setLessonIndex] = useState(() => {
    ...
    const index = dailyLessons.findIndex((item) => item.id === lessonId);
    return index === -1 ? 2 : index;
  });
  const [selected, setSelected] = useState<string | null>(null);
  const [checked, setChecked] = useState(false);
  const [completed, setCompleted] = useState<string[]>(() => loadLearningRoutine().completedIds);
  const [routine, setRoutine] = useState<LearningRoutine>(() => loadLearningRoutine());
  ...
  const archiveHref = import.meta.env.VITE_GITHUB_PAGES === "true" ? `${import.meta.env.BASE_URL}?view=archive` : `${import.meta.env.BASE_URL}archive`;
  ...
  useEffect(() => { const savedTheme = loadCharacterTheme(); setTheme(savedTheme); setHeroSceneIndex(friendHeroSceneIndex[savedTheme]); return () => { if (sceneTimer.current) window.clearTimeout(sceneTimer.current); }; }, []);
  useEffect(() => {
  ...
  const selectLesson = (index: number) => {
    ...
    const updateLesson = () => { setLessonIndex(index); setSelected(null); setChecked(false); setCompletionVisible(false); speak(`${next.weekday}요일 ${next.title} 학습지가 준비됐어요. ${characters[next.character].greeting}`, next.character); };
    updateLesson();
  };
  const handleLessonTabKeyDown = (event: React.KeyboardEvent<HTMLButtonElement>, index: number) => {
    const lastIndex = dailyLessons.length - 1;
    const nextIndex = event.key === "ArrowRight" ? (index + 1) % dailyLessons.length
      : event.key === "ArrowLeft" ? (index - 1 + dailyLessons.length) % dailyLessons.length
        : event.key === "Home" ? 0
    ...
    lessonTabRefs.current[nextIndex]?.focus();
  };
  ...
  const openLesson = () => { setRoutine((current) => { const next = markLessonOpened(current, lesson.id); saveLearningRoutine(next); return next; }); setLiveMessage(`${lesson.weekday}요일 학습지 봉투를 열었어요.`); speak(character.greeting, lesson.character); };
  const confirmScreenTactile = () => { setRoutine((current) => { const next = markTactileExplored(current, lesson.id); saveLearningRoutine(next); return next; }); setLiveMessage("화면의 점자 미리보기로 촉각 탐색을 이어가요."); };
  const saveForReview = () => { setRoutine((current) => { const next = markLessonForReview(current, lesson.id); saveLearningRoutine(next); return next; }); setLiveMessage(`${lesson.weekday}요일 학습지를 보관함의 다시 만져 보기 목록에 담았어요.`); };
  const handleDotPadFrameSent = () => { setRoutine((current) => { const next = markDotPadFrameSent(current, lesson.id); saveLearningRoutine(next); return next; }); setLiveMessage("DotPad에 오늘의 촉각 프레임을 보냈어요."); };
  const check = () => { if (!selected) return speak("먼저 답 하나를 골라 보세요."); setChecked(true); if (selected === lesson.answer) { const nextCompleted = completed.includes(lesson.id) ? completed : [...completed, lesson.id]; setCompleted(nextCompleted); setRoutine((current) => { const next = markLessonCompleted(current, lesson.id); saveLearningRoutine(next); return next; }); setCompletionVisible(true); setCelebrating(lesson.character); window.setTimeout(() => setCelebrating(null), 760); speak(characters[lesson.character].correct); } else { saveForReview(); speak(characters[lesson.character].retry); } };
  const isDone = completed.includes(lesson.id);
  const isOpened = routine.openedIds.includes(lesson.id);
  const isTactileExplored = routine.tactileExploredIds.includes(lesson.id);
  const streak = getCurrentStreak(completed, dailyLessons.map((item) => item.id));
  return <div className="garden-site" id="top" style={pageSceneStyle}><a className="sr-only" href="#today">오늘의 학습지로 바로가기</a><div className="sr-only" aria-live="polite">{liveMessage}</div>
    ...
    <main><section className="garden-hero" aria-labelledby="hero-title"><div className={`page-width hero-grid scene-${scenePhase} ${isDone ? "is-complete" : ""}`} style={heroSceneStyle}><span className="scene-fade-layer" aria-hidden="true" /><span className="daylight-layer" aria-hidden="true" /><span className="night-stars" aria-hidden="true"><i /><i /><i /><i /></span><div className="hero-copy"><span className="capsule-label"><i /><i /><i /> TODAY'S SHEET <b>{String(lessonIndex + 1).padStart(2, "0")}</b></span><h1 id="hero-title">매일 한 장,<br /><em>피어나는</em><br />오늘의 공부.</h1><p>오늘의 학습지가 닷패드에 도착해요. 만지고, 듣고, 답을 찾으며 내 감각으로 한 걸음씩 자라요.</p><div className="hero-actions"><button className="ink-button" onClick={() => goTo("today")}>오늘의 학습지 열기 <ArrowRight size={17} /></button><a className="paper-button" href={reportHref}>보호자 리포트 보기</a></div><div className="theme-picker" role="group" aria-label="Sensory 친구와 색상 테마 선택"><span>오늘의 친구</span><div>{(Object.keys(characters) as CharacterKey[]).map((key) => { const item = characters[key]; return <button key={key} className={theme === key ? "selected" : ""} onClick={() => selectFriend(key)} aria-pressed={theme === key}><span className="friend-avatar" aria-hidden="true" data-character={key}><img src={CURRICULUM_CHARACTERS[key]} alt="" /></span><b>{item.name}</b></button>; })}</div></div><span className="micro-copy">7 DAYS · 7 NEW TACTILE MOMENTS</span></div><div className="hero-garden" role="region" aria-label={`${characters[theme].name} 친구의 3D 장면: ${heroScene.label}`}><span className="hero-scene-label" data-testid="hero-scene-label">{isDone ? "SUNSET COMPLETE · " : ""}{characters[theme].name} · {heroScene.label}</span><div className="garden-sun" /><button className="character-tap hero-char one" onClick={() => speak(characters.pio.greeting)} aria-label="피오의 안내 듣기"><ShapeFace color="sky" kind="arch" /></button><button className="character-tap hero-char two" onClick={() => speak(characters.lulu.greeting)} aria-label="루루의 안내 듣기"><ShapeFace color="pink" kind="drop" /></button><button className="character-tap hero-face" onClick={() => speak(characters.momo.greeting)} aria-label="모모의 안내 듣기"><ShapeFace color="coral" kind="round" /></button><div className="hero-pad"><span className="pad-label">THIS WEEK · 7 NEW SHEETS</span><span className="pad-line"><i /><i /><i /><i /><i /><i /><i /></span><span className="pad-line short"><i /><i /><i /><i /></span></div></div></div></section>
      ...
      <section id="today" className="daily-section" aria-labelledby="today-title" style={{ "--arrival-scene": `url(${CLAY_ENVELOPE})` } as CSSProperties}>
        <div className="page-width">
          <div className="section-copy"><span className="capsule-label"><i /><i /><i /> THIS WEEK'S SHEETS <b>7</b></span><h2 id="today-title">매일 새로운 한 장을<br />손끝에 받아 보세요.</h2><p>월요일부터 일요일까지, 문해·수학·영어·촉각 그림을 번갈아 만나며 매일 다른 감각 경험을 이어가요.</p></div>
          <div className="week-picker" role="tablist" aria-label="7일 학습지 선택">
            {dailyLessons.map((item, index) => <button ref={(element) => { lessonTabRefs.current[index] = element; }} id={`lesson-tab-${item.id}`} key={item.id} role="tab" aria-selected={lessonIndex === index} aria-controls="daily-task-panel" tabIndex={lessonIndex === index ? 0 : -1} className={`${lessonIndex === index ? "active" : ""} ${completed.includes(item.id) ? "done" : ""}`} onClick={() => selectLesson(index)} onKeyDown={(event) => handleLessonTabKeyDown(event, index)} style={{ "--week-scene": `url(${WEEK_SCENES[index].src})` } as CSSProperties}><span>{item.weekday}</span><b>{completed.includes(item.id) ? <Check size={13} /> : String(index + 1)}</b><small>{item.subject}</small></button>)}
          </div>
          {!isOpened ? <DailyArrival lesson={lesson} dayNumber={lessonIndex + 1} tabId={`lesson-tab-${lesson.id}`} onOpen={openLesson} /> : <div className="daily-layout" style={lessonSceneStyle}>
            <aside className={`daily-intro ${lesson.accent}`}><span>{lesson.weekday}요일 · DAY {lessonIndex + 1}</span><small className="daily-scene-label">3D SCENE · {scene.label}</small><button className={`character-tap daily-face ${celebrating === lesson.character ? "celebrate" : ""}`} onClick={() => speak(character.greeting, lesson.character)} aria-label={`${character.name}의 안내 듣기`}><ShapeFace color={character.accent} kind={character.shape} character={lesson.character} sparkle={celebrating === lesson.character} /></button><h3>{lesson.title}</h3><p>{lesson.subject} · 오늘의 감각 미션</p><ol><li className="done"><b>1</b> 새 학습지가 도착했어요</li><li className={selected ? "done" : ""}><b>2</b> 손끝으로 점을 확인해요</li><li className={isDone ? "done" : ""}><b>3</b> 오늘의 “아하!”를 모아요</li></ol><button className="character-message" onClick={() => speak(character.greeting, lesson.character)}><Volume2 size={14} /><span><b>{character.name}</b> {character.role}<small>눌러서 안내 듣기</small></span></button></aside>
            <div id="daily-task-panel" className="daily-task" role="tabpanel" tabIndex={0} aria-labelledby={`lesson-tab-${lesson.id}`}>
              <div className="task-bar"><span><b>{String(lessonIndex + 1).padStart(2, "0")}</b> {lesson.subject.toUpperCase()} · DAILY QUIZ</span><button onClick={() => { const next = !voiceOn; setVoiceOn(next); if (!next && "speechSynthesis" in window) { speechToken.current += 1; window.speechSynthesis.cancel(); } setLiveMessage(next ? "음성 안내를 켰어요." : "음성 안내를 껐어요."); }} aria-pressed={voiceOn}>{voiceOn ? <Volume2 size={15} /> : <VolumeX size={15} />}{voiceOn ? "소리 켜짐" : "소리 꺼짐"}</button></div>
              <div className="question-block"><span>{lessonIndex === 2 ? "COUNT THE DOTS" : "FEEL THE DOTS"}</span><h3>{lesson.prompt}</h3><p>{lesson.description}</p></div>
              <section className="tactile-goal-card" aria-labelledby="tactile-goal-title"><span>TODAY&apos;S TACTILE GOAL</span><div><h4 id="tactile-goal-title">{lesson.tactileGoal.action} · {lesson.tactileGoal.concept}</h4><p>{lesson.tactileGoal.instruction}</p></div><small>{lesson.tactileGoal.complexity}</small></section>
              <div className="dotpad-preview"><span><small>DOTPAD · TACTILE PREVIEW</small><b>Liblouis 표준 점역 결과</b></span><div className="multi-dot-line">{lessonDots.map((dots, index) => <DotGrid dots={dots} key={`${lesson.id}-${index}`} />)}</div></div>
              <div className="tactile-mission" aria-labelledby="tactile-mission-title"><span id="tactile-mission-title">TODAY&apos;S TACTILE MISSION · 3 STEPS</span><ol><li className="done"><b>1</b><span>학습지 열기<small>오늘의 한 장을 받았어요.</small></span></li><li className={isTactileExplored ? "done" : "active"}><b>2</b><span>점을 만져 보기<small>{routine.dotpadFrameIds.includes(lesson.id) ? "DotPad에 프레임을 보냈어요." : "DotPad 또는 화면 점자를 선택해요."}</small></span></li><li className={isDone ? "done" : isTactileExplored ? "active" : ""}><b>3</b><span>답과 도장<small>정답을 고르고 오늘의 도장을 받아요.</small></span></li></ol>{!isTactileExplored ? <div className="tactile-choice"><p>DotPad가 없다면 화면 점자 미리보기와 음성 안내를 이용해도 다음 단계로 갈 수 있어요.</p><button type="button" onClick={confirmScreenTactile}>화면 점자로 계속하기</button></div> : null}</div>
              <DotPadConnection dots={lessonDots} lessonLabel={`${lesson.weekday}요일 ${lesson.title}`} onFrameSent={handleDotPadFrameSent} />
              <div className="answer-options" role="group" aria-label="답 선택지">{lesson.options.map((option, index) => { const picked = option === selected; const state = checked ? option === lesson.answer ? "correct" : picked ? "wrong" : "" : picked ? "picked" : ""; const colors = ["yellow", "pink", "sky"]; return <button className={`answer-option ${state}`} key={option} onClick={() => choose(option)} disabled={isDone} aria-pressed={picked}><ShapeFace color={colors[index]} kind="mini" /><b>{option}</b></button>; })}</div>
              {isDone ? completionVisible ? <CompletionStamp lesson={lesson} streak={streak} archiveHref={archiveHref} onNext={() => selectLesson(Math.min(lessonIndex + 1, dailyLessons.length - 1))} onReview={() => { saveForReview(); setCompletionVisible(false); }} /> : <div className="success-note"><span>✦</span><div><b>{character.name}와 오늘의 학습지를 완료했어요.</b><p>{character.correct}</p><button type="button" onClick={() => setCompletionVisible(true)}>완료 도장 보기</button></div></div> : <div className="answer-footer"><p className={checked ? selected === lesson.answer ? "yes" : "no" : ""}>{checked ? selected === lesson.answer ? "맞았어요. 캐릭터의 응원을 들어 보세요!" : "아직 아니에요. 보관함의 다시 만져 보기 목록에 담았어요." : "글자를 고른 뒤 정답을 확인해 보세요."}</p>{checked && selected !== lesson.answer ? <button onClick={() => speak(lesson.hint, lesson.character)}><Headphones size={15} />힌트 듣기</button> : <button onClick={check}>정답 확인 <ChevronRight size={15} /></button>}</div>}
            </div>
          </div>}
        </div>
      </section>
      <section id="how" className="how-section" aria-labelledby="how-title"><div className="page-width how-grid"><div className="how-copy"><span className="capsule-label"><i /><i /><i /> WHY SENSORY? <b>?</b></span><h2 id="how-title">보고, 듣고,<br />만지며<br /><em>매일 자라요.</em></h2><p>문제를 풀 때마다 캐릭터가 나에게 맞는 말로 반응하고, 완료 기록은 다음 학습지와 보호자 리포트로 이어집니다.</p><a className="paper-dark-button" href={reportHref}>보호자 리포트 보기 <ArrowRight size={16} /></a></div><div className="growth-poster" style={{ "--card-scene": `url(${CURRICULUM_SCENES.nabi})` } as CSSProperties}><button className="character-tap poster-face featured-character" onClick={() => speak(characters.nabi.greeting)} aria-label="나비의 안내 듣기"><img className="featured-character-image" src={CURRICULUM_CHARACTERS.nabi} alt="촉각 지도를 살피는 나비" /></button><p>TOUCH,<br />THEN GROW!</p></div></div></section>
      <section id="curriculum" className="curriculum-section" aria-labelledby="curriculum-title"><div className="page-width"><div className="curriculum-header"><div><span className="capsule-label"><i /><i /><i /> TACTILE GARDEN <b>04</b></span><h2 id="curriculum-title">7일 동안 만나는<br />작은 학습 정원.</h2></div><p>문해·수학·영어·촉각 그림이 매일의 한 장 안에서 자연스럽게 이어집니다.</p></div><div className="curriculum-grid"><article className="curriculum-card purple" style={{ "--card-scene": `url(${CURRICULUM_SCENES.momo})` } as CSSProperties}><img className="card-character card-character-image" src={CURRICULUM_CHARACTERS.momo} alt="점자 타일을 든 모모" /><div className="curriculum-copy"><span>MON · TUE · SUN</span><h3>점자 문해</h3><p>점자 칸과 친해지고 익숙한 단어를 읽어요.</p></div></article><article className="curriculum-card yellow" style={{ "--card-scene": `url(${CURRICULUM_SCENES.pio})` } as CSSProperties}><img className="card-character card-character-image" src={CURRICULUM_CHARACTERS.pio} alt="수 세기 블록을 살피는 피오" /><div className="curriculum-copy"><span>WED</span><h3>수학</h3><p>수와 연산의 관계를 손끝으로 세어 봐요.</p></div></article><article className="curriculum-card pink" style={{ "--card-scene": `url(${CURRICULUM_SCENES.lulu})` } as CSSProperties}><img className="card-character card-character-image" src={CURRICULUM_CHARACTERS.lulu} alt="알파벳 책을 든 루루" /><div className="curriculum-copy"><span>THU</span><h3>영어</h3><p>소리와 철자를 함께 점자로 연결해요.</p></div></article><article className="curriculum-card lime" style={{ "--card-scene": `url(${CURRICULUM_SCENES.nabi})` } as CSSProperties}><img className="card-character card-character-image" src={CURRICULUM_CHARACTERS.nabi} alt="촉각 지도를 살피는 나비" /><div className="curriculum-copy"><span>FRI · SAT</span><h3>촉각 그림</h3><p>도형과 지도 속 정보를 손끝으로 읽어요.</p></div></article></div></div></section>
      <section id="studio" className="studio-section" aria-labelledby="studio-title"><div className="page-width studio-grid"><div className="studio-copy"><span className="capsule-label"><i /><i /><i /> BRAILLE LAB <b>+ +</b></span><h2 id="studio-title">내가 쓴 말이<br /><em>점점점</em> 하고<br />올라와요.</h2><p>내 이름이나 좋아하는 단어를 적어 보세요. Sensory가 Liblouis 표준 점역으로 바꿔 닷패드 한 줄에 보여 줍니다.</p></div><div className="studio-console"><div className="console-title"><span><Send size={15} /> LIVE DOTPAD PREVIEW</span><b>LIBLOUIS</b></div><label className="sr-only" htmlFor="studio-input">점자로 바꿀 한글, 영어, 숫자를 입력</label><div className="console-input"><input id="studio-input" value={studioText} maxLength={20} onChange={(event) => { setStudioText(event.target.value); setLiveMessage("Liblouis 점역 미리보기를 갱신하고 있어요."); }} placeholder="예: 내 이름" /><button onClick={() => speak(studioText || "입력한 글자가 없습니다.")} aria-label="입력한 글자 듣기"><Volume2 size={18} /></button></div><div className="studio-dotpad" role="img" aria-label={`${studioText || "점자"} 표준 점역 촉각 미리보기`}>{studioCells.map((cell, index) => <DotGrid compact dots={cell} key={`${index}-${cell.join("-")}`} />)}</div><div className="console-footer"><span>{brailleNotice}</span><button onClick={() => { setStudioText("센서리"); speak("센서리 예시로 되돌렸어요."); }}>예시로 돌아가기 <ChevronRight size={14} /></button></div></div></div></section></main>
    <nav className="mobile-dock" aria-label="모바일 빠른 탐색"><a href="#top"><House size={18} /><span>홈</span></a><button onClick={() => goTo("today")}><BookOpenCheck size={18} /><span>학습</span></button><button onClick={() => goTo("studio")}><PanelsTopLeft size={18} /><span>점자</span></button><a href={reportHref}><ChartNoAxesCombined size={18} /><span>리포트</span></a></nav>
    <footer className="garden-footer"><div className="page-width footer-row"><div><span className="capsule-label"><i /><i /><i /> SEE YOU TOMORROW <b>✦</b></span><h2>내일도<br />새로운 한 장을 만나.</h2></div><a className="footer-button" href={reportHref}>보호자 리포트 보기 <ArrowRight size={16} /></a></div><section className="page-width dotpad-inquiry" aria-labelledby="dotpad-inquiry-title"><div className="dotpad-inquiry-copy"><span className="capsule-label"><i /><i /><i /> DOTPAD WITH SENSORY <b>01</b></span><h3 id="dotpad-inquiry-title">집에서도 학교에서도,<br />DotPad를 만나 보세요.</h3><p>렌탈·구매 가능 여부와 기관 도입, 데모 상담은 Dot Inc. 공식 문의 페이지에서 확인할 수 있어요.</p></div><div className="dotpad-inquiry-actions"><a className="dotpad-inquiry-primary" href="https://www.dotincorp.com/en/contact" target="_blank" rel="noreferrer"><MailOpen size={19} aria-hidden="true" /><span>DotPad 렌탈·구매 문의하기<small>Dot Inc. 공식 문의 페이지 열기</small></span><ArrowRight size={18} aria-hidden="true" /></a><a className="dotpad-product-link" href="https://www.dotincorp.com/en/product/dotpadx" target="_blank" rel="noreferrer">Dot Pad X 자세히 보기 <ArrowRight size={15} aria-hidden="true" /></a></div></section><div className="page-width footer-bottom"><span className="garden-brand"><b>sensory</b></span><span>닷패드 연동 촉각 학습 경험 컨셉 데모</span></div></footer></div>;
}
