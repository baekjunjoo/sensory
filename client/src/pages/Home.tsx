...
import { getCurrentStreak, loadLearningRoutine, markDotPadFrameSent, markLessonCompleted, markLessonOpened, markTactileExplored, saveLearningRoutine, type LearningRoutine } from "@/lib/dailyRoutine";
        ...
        <div className="page-width">
          ...
          {!isOpened ? <DailyArrival lesson={lesson} dayNumber={lessonIndex + 1} tabId={`lesson-tab-${lesson.id}`} onOpen={openLesson} /> : <div className="daily-layout" style={lessonSceneStyle}>
            ...
            <div id="daily-task-panel" className="daily-task" role="tabpanel" tabIndex={0} aria-labelledby={`lesson-tab-${lesson.id}`}>
              ...
              {isDone ? completionVisible ? <CompletionStamp lesson={lesson} streak={streak} archiveHref={archiveHref} onNext={() => selectLesson(Math.min(lessonIndex + 1, dailyLessons.length - 1))} onReview={() => setCompletionVisible(false)} /> : <div className="success-note"><span>✦</span><div><b>{character.name}와 오늘의 학습지를 완료했어요.</b><p>{character.correct}</p><button type="button" onClick={() => setCompletionVisible(true)}>완료 도장 보기</button></div></div> : <div className="answer-footer"><p className={checked ? selected === lesson.answer ? "yes" : "no" : ""}>{checked ? selected === lesson.answer ? "맞았어요. 캐릭터의 응원을 들어 보세요!" : "아직 아니에요. 손끝의 점을 다시 따라가 볼까요?" : "글자를 고른 뒤 정답을 확인해 보세요."}</p>{checked && selected !== lesson.answer ? <button onClick={() => speak(lesson.hint, lesson.character)}><Headphones size={15} />힌트 듣기</button> : <button onClick={check}>정답 확인 <ChevronRight size={15} /></button>}</div>}
            </div>
          </div>}
        </div>
      </section>
