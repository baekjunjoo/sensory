...
describe("오늘의 친구 프로필 레이아웃", () => {
  it("원형 3D 얼굴·하단 이름·모바일 크기를 명시한다", () => {
    ...
    expect(styles).toContain("background-image:var(--friend-momo)");
    expect(styles).toContain("background-image:var(--friend-pio)");
    expect(styles).toContain("background-image:var(--friend-lulu)");
    expect(styles).toContain("background-image:var(--friend-nabi)");
    expect(styles).toContain(".friend-avatar .sensory-blob{display:none!important}");
